use super::{
    Fragment, frame::{Access, Frame}, symbols::{Label, Temp}
};
...
fn un_ex(expr: Expr) -> ir::Expr {
    match expr {
        ...
        Expr::Nx(stmt) => ir::Expr::ESeq(Box::new(stmt), Box::new(ir::Expr::Const(0))),
        Expr::Cx(gen_stmt) => {
            let r = Temp::new();
            let t = Label::new();
            let f = Label::new();
            ...
            ir::Expr::ESeq(Box::new(stmts), Box::new(ir::Expr::Temp(r)))
        }
    }
}
...
fn un_nx(expr: Expr) -> ir::Stmt {
    match expr {
        ...
        Expr::Cx(gen_stmt) => {
            ...
            ir::Stmt::Seq(Box::new(statement), Box::new(ir::Stmt::Label(join)))
        }
    }
}
...
fn fold_stmts<I: IntoIterator<Item = ir::Stmt>>(stmts: I) -> Option<ir::Stmt> {
    ...
    let first = iter.next()?;
    let stmt = iter.fold(first, |acc, s| ir::Stmt::Seq(Box::new(acc), Box::new(s)));
...
pub fn chain_expr(front: Expr, back: Expr) -> Expr {
    ...
    Expr::Ex(ir::Expr::ESeq(Box::new(stmt), Box::new(expr)))
}
...
pub fn if_else(cond: Expr, then_b: Expr, else_b: Expr) -> Expr {
    ...
    Expr::Ex(ir::Expr::ESeq(Box::new(stmts), Box::new(ir::Expr::Temp(r))))
}
...
fn mem(access: &Access) -> ir::Expr {
    match access {
        Access::InFrame(offset) => ir::Expr::Mem(Box::new(ir::Expr::BinOp(
            ir::BinOp::Add,
            Box::new(ir::Expr::Temp(Temp::FP)),
            Box::new(ir::Expr::Const(*offset as i64)),
...
fn arithmetic_op(op: ir::BinOp, l: Expr, r: Expr) -> Expr {
    Expr::Ex(ir::Expr::BinOp(op, Box::new(un_ex(l)), Box::new(un_ex(r))))
}
...
pub fn print_int(e: Expr) -> Expr {
    ...
    Expr::Ex(ir::Expr::Call(
        Box::new(ir::Expr::Name(print_label)),
        vec![un_ex(e)],
    ))
}
...
pub fn fun_app(fun: Expr, args: Vec<Expr>) -> Expr {
    ...
    Expr::Ex(ir::Expr::Call(Box::new(un_ex(fun)), ir_args))
}
...
impl FunContext {
    ...
    pub fn exit(self, body_ir: Expr) -> Fragment {
        ...
        let full_body = match self.prolog {
            Some(p) => ir::Expr::ESeq(Box::new(p), Box::new(body)),
            None => body,
        };
