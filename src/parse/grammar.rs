...
peg::parser! {
    pub grammar lang() for str {
        ...
        #[cache_left_rec]
        rule _term() -> ast::Term
            // let: 4 variants (continuation yes/no, type annotation yes/no).
            // Missing continuation defaults to Unit.
            = "let" spa() __ i:ident() __ "=" __ t:term() _ "\n" __ c:term()
              { ast::Term::Let(i, None, t, c) }
            / "let" spa() __ s:position!() i:ident() e:position!() __ "=" __ t:term()
              { ast::Term::Let(i, None, t, Tag::new(ast::Term::Unit, (s, e))) }
            / "let" spa() __ i:ident() _ ":" _ ty:typ() __  "=" __ t:term() _ "\n" __ c:term()
              { ast::Term::Let(i, Some(ty), t, c) }
            / "let" spa() __ s:position!() i:ident() e:position!()  _ ":" _ ty:typ() __ "=" __ t:term()
            ...
            // Missing continuation defaults to Unit.
            / "fn" spa() i:ident() _ "(" _ ts:list(<tuple_colon(<ident()>, <typ()>)>) _ ")" _ "->" _ ty:typ() __ "{" __ t:term() __ "}" _ "\n" __ c:term()
              { ast::Term::FunDec(i, ts, ty, t, c) }
            / "fn" spa() i:ident() _ "(" _ ts:list(<tuple_colon(<ident()>, <typ()>)>) _ ")" _ "->" _ ty:typ() __ "{" __ t:term() __ "}"
              { ast::Term::FunDec(i.clone(), ts, ty, t, Tag::new(ast::Term::Unit, i.span)) }
            / "fn" spa() i:ident() _ "(" _ ts:list(<tuple_colon(<ident()>, <typ()>)>) _ ")" _ "{" __ t:term() __ "}" _ "\n" __ c:term()
              { ast::Term::FunDec(i.clone(), ts, Tag::new(ast::Type::Unit, i.span), t, c) }
            / "fn" spa() i:ident() _ "(" _ ts:list(<tuple_colon(<ident()>, <typ()>)>) _ ")" _ "{" __ t:term() __ "}"
        ...
        rule top() -> ast::_Top = precedence! {
            s:position!() t:@ e:position!() { Tag::new(t, (s, e)) }
            --
            "fn" spa() i:ident() _ "(" __ ts:list(<tuple_colon(<ident()>, <typ()>)>) __ ")" _ "->" _ ty:typ() __ "{" __ t:term() __ "}"
            { ast::Top::FunDec(i, ts, ty, t) }

            "fn" spa() i:ident() _ "(" __ ts:list(<tuple_colon(<ident()>, <typ()>)>) __ ")" _ "{" __ t:term() __ "}"
